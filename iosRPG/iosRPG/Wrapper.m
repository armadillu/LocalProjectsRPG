...
- (NSError*)startConnection:(NSURLRequest *)request;
@end
...
- (NSError*)sendRequestTo:(NSURL *)url usingVerb:(NSString *)verb withParameters:(NSDictionary *)parameters {
    ...
    [params release];
    NSError * error = [self startConnection:request];
	return error;
}
...
- (NSString *)responseAsText {
    NSString *resp = [[[NSString alloc] initWithData:self.receivedData
    ...
    if (self.asynchronous) {
        ...
        NSData *data = [NSURLConnection sendSynchronousRequest:request
                                             returningResponse:&response
                                                         error:&error];
		if (error != nil){
			NSLog(@"NSURLConnection error: %@", [error userInfo]);
			return error;
		}
